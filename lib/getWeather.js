...
export default async function getWeather(spinner) {
  ...
  console.log(`
    ${chalk.bgGreen(" " + weatherData.location.name + " " + weatherData.location.country + " ")}

    ${weatherData.current.condition.text} ${Math.floor(weatherData.current.temp_c)}°C
    Feels like ${weatherData.current.feelslike_c}°C

    ${chalk.bgRgb(255, 203, 164)(" " + upcomingHours[0].time.slice(-5) + " ")} ${upcomingHours[0].condition.text}, ${Math.floor(upcomingHours[0].temp_c)}°C
    ${chalk.bgRgb(255, 203, 164)(" " + upcomingHours[1].time.slice(-5) + " ")} ${upcomingHours[1].condition.text}, ${Math.floor(upcomingHours[1].temp_c)}°C
    ${chalk.bgRgb(255, 203, 164)(" " + upcomingHours[2].time.slice(-5) + " ")} ${upcomingHours[2].condition.text}, ${Math.floor(upcomingHours[2].temp_c)}°C
